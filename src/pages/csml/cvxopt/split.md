...
It belongs to a class of method known as *alternating direction methods*...

### DPR splitting for the kernel

Let's assume that $b=0$ for now and let $y=Cx=-Bx$.
Then, we can suggest a triplet of fixed-points:
...
Let now $u_k=y_k/\alpha$ and note that $Cz_{k+1}= (\alpha x_{k+1} + y_k - \alpha z_{k+1})$ using the second iteration.
...
Note that $(\partial f+\alpha \mathbf I) \equiv \alpha (\partial (\alpha\inv f)+\mathbf I)$ so that
